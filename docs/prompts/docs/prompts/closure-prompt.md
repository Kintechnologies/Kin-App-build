...
**Last updated:** 2026-04-05T00:00 (session 6)
...
### Scenario 5: Manual Resolution — Parent Closes the Issue
**Input:**
```json
{
...
```
**Pass criteria:** Acknowledges manual close without implying Kin validated coverage independently. Matter-of-fact. Specific (names child and time). 1 sentence. No enthusiasm. No generic "Coverage confirmed." Does not add a new coordination question — the parent made the call; Kin acknowledges and clears.

**Note:** MANUAL_RESOLVED is the only resolution type where Kin does not know what actually happened — a parent explicitly closed the issue. The closure line should reflect that the parent resolved it, not that coverage has been arranged or confirmed by data.
...
6. **MANUAL_RESOLVED treated as COVERAGE_CONFIRMED** — Model says "Maya's 3:30 is covered" when the parent just manually closed the issue without confirming actual coverage. Fix: resolution_type in input; prompt instructs that MANUAL_RESOLVED = parent called it done, not that Kin confirmed coverage. Output should acknowledge close, not assert coverage.
