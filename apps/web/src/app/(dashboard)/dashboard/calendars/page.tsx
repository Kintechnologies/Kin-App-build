"use client";

import { useEffect, useState } from "react";
import {
  Plus,
  Trash2,
  Loader2,
  RefreshCw,
  CalendarCheck,
  ShieldCheck,
} from "lucide-react";

// ── Types ─────────────────────────────────────────────────────────────────────

interface CalendarConnection {
  id: string;
  provider: "google" | "apple";
  email?: string | null;
  enabled?: boolean;
  sync_status?: "idle" | "syncing" | "error";
  sync_error?: string | null;
  last_synced_at?: string | null;
}

// ── Atoms ─────────────────────────────────────────────────────────────────────

function MonoLabel({ children }: { children: React.ReactNode }) {
  return (
    <span
      className="font-mono uppercase"
      style={{
        fontSize: "11.5px",
        letterSpacing: "0.04em",
        color: "var(--warm-40)",
      }}
    >
      {children}
    </span>
  );
}

function ProviderGlyph({ provider }: { provider: "google" | "apple" }) {
  if (provider === "google") {
    return (
      <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
      </svg>
    );
  }
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#2C2C28" aria-hidden="true">
      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
    </svg>
  );
}

function StatusDot({ status }: { status: "connected" | "syncing" | "error" }) {
  const color =
    status === "connected"
      ? "#5C6B4F"
      : status === "syncing"
        ? "#7A8C6A"
        : "#A65A4A";
  return (
    <span
      aria-hidden="true"
      style={{
        display: "inline-block",
        width: "6px",
        height: "6px",
        borderRadius: "50%",
        background: color,
        flexShrink: 0,
      }}
    />
  );
}

function relativeTime(iso?: string | null): string {
  if (!iso) return "Not yet synced";
  const then = new Date(iso).getTime();
  const mins = Math.round((Date.now() - then) / 60000);
  if (mins < 1) return "Synced just now";
  if (mins < 60) return `Synced ${mins}m ago`;
  const hrs = Math.round(mins / 60);
  if (hrs < 24) return `Synced ${hrs}h ago`;
  const days = Math.round(hrs / 24);
  return `Synced ${days}d ago`;
}

// ── Page ──────────────────────────────────────────────────────────────────────

export default function CalendarsPage() {
  const [connections, setConnections] = useState<CalendarConnection[]>([]);
  const [loading, setLoading] = useState(true);
  const [connecting, setConnecting] = useState(false);
  const [syncing, setSyncing] = useState(false);
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/calendar/sync");
        if (res.ok && !cancelled) {
          const { connections: conns } = await res.json();
          if (Array.isArray(conns)) setConnections(conns as CalendarConnection[]);
        }
      } catch {
        /* non-fatal — empty state renders */
      } finally {
        if (!cancelled) setLoading(false);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, []);

  async function connectGoogle() {
    setConnecting(true);
    setError(null);
    try {
      const res = await fetch("/api/calendar/google");
      const data = (await res.json()) as { url?: string; error?: string };
      if (res.ok && data.url) {
        window.location.href = data.url;
        return;
      }
      setError(data.error ?? "Could not start the calendar connection.");
    } catch {
      setError("Network error. Please try again.");
    }
    setConnecting(false);
  }

  async function syncNow() {
    setSyncing(true);
    setError(null);
    try {
      await fetch("/api/calendar/sync", { method: "POST" });
      const res = await fetch("/api/calendar/sync");
      if (res.ok) {
        const { connections: conns } = await res.json();
        if (Array.isArray(conns)) setConnections(conns as CalendarConnection[]);
      }
    } catch {
      setError("Could not refresh calendars. Please try again.");
    }
    setSyncing(false);
  }

  async function disconnect(conn: CalendarConnection) {
    setConnections((prev) => prev.filter((c) => c.id !== conn.id));
    setConfirmId(null);
    const endpoint =
      conn.provider === "google"
        ? "/api/calendar/google"
        : "/api/calendar/apple/connect";
    try {
      await fetch(endpoint, { method: "DELETE" });
    } catch {
      /* optimistic — already removed from view */
    }
  }

  return (
    <div
      style={{
        maxWidth: "640px",
        margin: "0 auto",
        padding: "32px clamp(20px, 4vw, 32px) 56px",
        display: "flex",
        flexDirection: "column",
        gap: "20px",
      }}
    >
      <header>
        <MonoLabel>{"// CALENDARS"}</MonoLabel>
        <h1
          style={{
            fontFamily: "var(--font-instrument-serif), 'Playfair Display', serif",
            fontSize: "34px",
            fontWeight: 400,
            color: "var(--warm)",
            letterSpacing: "-0.015em",
            marginTop: "8px",
          }}
        >
          Calendar management
        </h1>
        <p
          style={{
            fontSize: "13.5px",
            color: "var(--warm-56)",
            marginTop: "6px",
            lineHeight: 1.55,
            maxWidth: "440px",
          }}
        >
          Kin reads these calendars each morning to build your briefing and
          spot pickup conflicts before they happen.
        </p>
      </header>

      <section
        style={{
          background: "#FDFBF7",
          border: "0.5px solid var(--hair)",
          borderRadius: "8px",
          padding: "20px",
          display: "flex",
          flexDirection: "column",
          gap: "14px",
        }}
      >
        <div
          style={{
            display: "flex",
            alignItems: "center",
            justifyContent: "space-between",
            gap: "12px",
          }}
        >
          <h2
            style={{
              fontSize: "15px",
              fontWeight: 500,
              color: "var(--warm)",
              letterSpacing: "-0.01em",
            }}
          >
            Connected calendars
          </h2>
          {connections.length > 0 && (
            <button
              onClick={syncNow}
              disabled={syncing}
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: "6px",
                background: "transparent",
                border: "none",
                color: "var(--warm-56)",
                fontSize: "12px",
                fontFamily: "var(--font-geist-mono), monospace",
                letterSpacing: "0.04em",
                textTransform: "uppercase",
                cursor: syncing ? "wait" : "pointer",
              }}
            >
              <RefreshCw
                size={12}
                className={syncing ? "animate-spin" : undefined}
              />
              {syncing ? "Syncing" : "Sync now"}
            </button>
          )}
        </div>

        {loading ? (
          <p
            style={{
              fontSize: "13px",
              color: "var(--warm-40)",
              display: "flex",
              alignItems: "center",
              gap: "6px",
            }}
          >
            <Loader2 size={12} className="animate-spin" /> Loading calendars…
          </p>
        ) : connections.length === 0 ? (
          <div
            style={{
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
              textAlign: "center",
              gap: "10px",
              padding: "24px 16px",
              border: "0.5px dashed var(--hair-strong)",
              borderRadius: "8px",
            }}
          >
            <div
              style={{
                width: "40px",
                height: "40px",
                borderRadius: "8px",
                background: "var(--sage-12)",
                border: "0.5px solid var(--hair-sage)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                color: "var(--sage)",
              }}
            >
              <CalendarCheck size={18} strokeWidth={1.8} />
            </div>
            <div>
              <div
                style={{
                  fontSize: "14px",
                  color: "var(--warm)",
                  fontWeight: 500,
                  marginBottom: "3px",
                }}
              >
                No calendars connected yet
              </div>
              <div
                style={{
                  fontSize: "12.5px",
                  color: "var(--warm-56)",
                  lineHeight: 1.5,
                  maxWidth: "320px",
                }}
              >
                Connect a calendar so Kin can start writing your morning
                briefings.
              </div>
            </div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {connections.map((conn) => {
              const status =
                conn.sync_status === "error"
                  ? "error"
                  : conn.sync_status === "syncing"
                    ? "syncing"
                    : "connected";
              const isConfirming = confirmId === conn.id;
              return (
                <div
                  key={conn.id}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "12px",
                    padding: "12px 14px",
                    background: "rgba(44,44,40,0.02)",
                    border: "0.5px solid var(--hair)",
                    borderRadius: "8px",
                  }}
                >
                  <div
                    style={{
                      width: "32px",
                      height: "32px",
                      borderRadius: "8px",
                      background: "var(--warm-06)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      flexShrink: 0,
                    }}
                  >
                    <ProviderGlyph provider={conn.provider} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: "8px",
                        marginBottom: "2px",
                      }}
                    >
                      <span
                        style={{
                          fontSize: "14px",
                          color: "var(--warm)",
                          fontWeight: 500,
                          overflow: "hidden",
                          textOverflow: "ellipsis",
                          whiteSpace: "nowrap",
                        }}
                      >
                        {conn.email ??
                          (conn.provider === "google"
                            ? "Google account"
                            : "Apple account")}
                      </span>
                      <StatusDot status={status} />
                    </div>
                    <MonoLabel>
                      {conn.provider} ·{" "}
                      {status === "error"
                        ? "Sync error"
                        : relativeTime(conn.last_synced_at)}
                    </MonoLabel>
                  </div>
                  {isConfirming ? (
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button
                        onClick={() => disconnect(conn)}
                        style={{
                          background: "rgba(166,90,74,0.1)",
                          border: "0.5px solid rgba(166,90,74,0.3)",
                          borderRadius: "4px",
                          padding: "6px 10px",
                          color: "#A65A4A",
                          fontSize: "12px",
                          fontWeight: 500,
                          cursor: "pointer",
                        }}
                      >
                        Disconnect
                      </button>
                      <button
                        onClick={() => setConfirmId(null)}
                        style={{
                          background: "transparent",
                          border: "0.5px solid var(--hair-strong)",
                          borderRadius: "4px",
                          padding: "6px 10px",
                          color: "var(--warm-56)",
                          fontSize: "12px",
                          cursor: "pointer",
                        }}
                      >
                        Keep
                      </button>
                    </div>
                  ) : (
                    <button
                      onClick={() => setConfirmId(conn.id)}
                      aria-label={`Disconnect ${conn.provider} calendar`}
                      style={{
                        background: "transparent",
                        border: "none",
                        color: "var(--warm-24)",
                        cursor: "pointer",
                        padding: "6px",
                        borderRadius: "6px",
                        display: "flex",
                        alignItems: "center",
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.color = "#A65A4A";
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.color = "var(--warm-24)";
                      }}
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
              );
            })}
          </div>
        )}

        <div>
          <button
            onClick={connectGoogle}
            disabled={connecting}
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: "7px",
              background: "transparent",
              border: "0.5px solid var(--hair-sage)",
              borderRadius: "4px",
              padding: "9px 14px",
              fontSize: "13px",
              fontWeight: 500,
              color: "var(--sage)",
              cursor: connecting ? "wait" : "pointer",
              opacity: connecting ? 0.6 : 1,
            }}
          >
            {connecting ? (
              <Loader2 size={14} className="animate-spin" />
            ) : (
              <Plus size={14} />
            )}
            Connect another calendar
          </button>
        </div>

        {error && (
          <p
            role="alert"
            style={{ fontSize: "12.5px", color: "rgba(166,90,74,0.9)" }}
          >
            {error}
          </p>
        )}
      </section>

      <div
        style={{
          display: "flex",
          alignItems: "flex-start",
          gap: "10px",
          padding: "13px 15px",
          background: "var(--sage-12)",
          border: "0.5px solid var(--hair-sage)",
          borderRadius: "8px",
        }}
      >
        <ShieldCheck
          size={15}
          style={{ color: "var(--sage)", flexShrink: 0, marginTop: "1px" }}
        />
        <p
          style={{
            fontSize: "12.5px",
            color: "var(--warm-72)",
            lineHeight: 1.55,
          }}
        >
          Read-only access. Kin reads your events to write the briefing — it
          never edits, deletes, or creates anything on your calendar.
        </p>
      </div>
    </div>
  );
}
