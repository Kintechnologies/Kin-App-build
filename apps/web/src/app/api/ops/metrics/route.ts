 ...
 * GET /api/ops/metrics
 *
 * Founder-facing snapshot of system health. Admin-only — gated by the
 * profiles.is_admin flag set in migration 062. Returns six sections that the
 * /ops dashboard renders as cards:
 *
 ...
 *   alerts     — last 20 entries from alerts_log
...
async function recentAlerts(
  ...
  const { data } = await supabase
    .from("alerts_log")
    .select("id, severity, category, message, created_at, delivered")
...
export async function GET() {
  ...
  const [briefing, sms, calendar, alerts, users] = await Promise.all([
